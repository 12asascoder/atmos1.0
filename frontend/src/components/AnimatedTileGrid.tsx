...
export default function AnimatedTileGrid() {
  const [tiles, setTiles] = useState<Tile[]>([]);
  const [images, setImages] = useState<string[]>([]);

  // 🔥 Load ALL images from src/assets/tiles automatically
  useEffect(() => {
      ...
      for (let c = 0; c < GRID_CONFIG.COLS; c++) {
        if (!GRADIENT_CELLS.some(g => g.row === r && g.col === c)) {
          imagePositions.push({
            row: r,
            col: c,
            imagePath: loadedImages[imagePositions.length % loadedImages.length],
          });
        }
      }
    }

    setTiles(imagePositions);
  }, []);
  ...
  const shuffle = useCallback(() => {
    ...
    setTiles(prev =>
      prev.map((tile, index) => ({
        ...tile,
        imagePath: shuffled[index % shuffled.length],
      }))
    );
  }, [images]);
  ...
  return (
    <div className="animated-grid-container">
      ...
      <style>{`
        ...
        .tile-grid {
          display: grid;
          width: min(95vw, calc((100vh - 90px) * ${GRID_CONFIG.COLS} / ${GRID_CONFIG.ROWS}));
          aspect-ratio: ${GRID_CONFIG.COLS} / ${GRID_CONFIG.ROWS};
        }
